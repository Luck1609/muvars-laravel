      ...
      <div className="w-full max-w-5xl rounded-md bg-blue-600 py-5 bg-opacity-90 z-10 mx-auto  lg:-mb-[90px]">
        <FormProvider {...methods}>
          <form className="w-full lg:grid lg:grid-cols-4 gap-3 p-5" onSubmit={handleSubmit(submit)}>
            <label className="block col-span-4 text-2xl font-semi-bold mb-3 text-white">Where should we take you?</label>
            <Input 
              name="origin"
              label="from where?"
            ...
            <FormBtn 
              content="Search"
              className="bg-rose-500 hover:bg-rose-600 w-full h-12 lg:h-auto"
            />
          </form>
        </FormProvider>
      </div>
